...
var Canvas = function (elem, opt, images) {
  ...
  for (i = 0; i < opt.view.width; i++) {
    if (opt.coordinates && opt.coordinates.top)
      this.ctx.fillText(
        C.COORDINATES[i + opt.view.xOffset],
  ...
  this.backup
    .getContext('2d')
    .drawImage(canvas, 0, 0, canvas.width, canvas.height, 0, 0, canvas.width, canvas.height);
  ...
  */
  ...
  jboard.each(
    function (c, type, mark) {
      ...
      if (type == C.CLEAR && mark && isLabel.test(mark)) clearFunc(ox, oy);
    }.bind(this),
    i1,
    j1,
    i2,
    j2
  ); // provide iteration limits
  ...
  jboard.each(
    function (c, type) {
      ...
      if (type == C.BLACK || type == C.WHITE) {
        this.stones.drawShadow(this.ctx, this.opt.shadow.xOff + ox, this.opt.shadow.yOff + oy);
      }
    }.bind(this),
    i1,
    j1,
