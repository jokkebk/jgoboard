  ...
  for (var i = 0, len = propValues.length; i < len; i++) {
    ...
    if (val.indexOf(':') == -1) {
      ...
      for (coord.i = c1.i; coord.i <= c2.i; ++coord.i)
        for (coord.j = c1.j; coord.j <= c2.j; ++coord.j) coords.push(coord.copy());
    }
  }
...
function sgfMove(node, name, values, moveMarks) {
  ...
  if (moveMarks && play.ko) node.setMark(play.ko, C.MARK.SQUARE);

  if (play.success && coord !== null) {
    node.setType(coord, player); // play stone
    node.setType(play.captures, C.CLEAR); // clear opponent's stones
    if (moveMarks) node.setMark(coord, C.MARK.CIRCLE);
...
function parseSGF(sgf) {
  ...
  for (i = 0, len = tokens.length; i < len; ++i) {
    ...
    if (bracketOpen == -1) {
      // handling elements outside property values (i.e. square brackets)
      token = token.replace(/^\s+|\s+$/g, ''); // trim whitespace, it is irrelevant here
      if (token == '[')
        // found one
        bracketOpen = i;
      else if (token !== '')
        // we are outside brackets, so just push everything nonempty as it is into 'processed'
        processed.push(token);
    } else {
      // bracket is open, we're now looking for a ] without preceding \
      if (token != ']') {
        // a text segment
        ...
        // a closing bracket
        if (lastBackslash) {
          // it's escaped - we continue
          lastBackslash = false;
        } else {
          // it's not escaped - we close the segment
          processed.push(
            tokens
              .slice(bracketOpen, i + 1)
              .join('')
              .replace(/\\\]/g, ']')
          ); // push the whole thing including brackets, and unescape the inside closing brackets
          bracketOpen = -1;
        }
      }
    }
  ...
  var node,
    propertyId = ''; // node under construction, and current property identifier
  ...
  for (i = 0, len = tokens.length; i < len; ++i) {
    ...
    if (token == '(' || token == ')') {
      if (node) {
        // flush and reset node if necessary
        if (propertyId !== '' && node[propertyId].length === 0) {
          // last property was missing value
      ...
      // it's either a property identifier or a property value
      if (token.indexOf('[') !== 0) {
        // it's property identifier
        if (propertyId !== '' && node[propertyId].length === 0) {
          // last property was missing value
          ERROR = 'Missing property value at ' + token + '!';
          return false;
  ...
  for (i = 1, len = tokens.length; i < len - 1; ++i) {
    ...
    if (token == '(') {
      ...
      if (currentRoot.sequence.length === 0) {
        // consecutive parenthesis without node sequence in between will throw an error
      ...
      // if every '(' is not followed by exactly one array of nodes (as it should), this code fails
...
function recurseRecord(jrecord, gameTree, moveMarks) {
  for (var i = 0; i < gameTree.sequence.length; i++) {
    ...
    for (var key in node) {
      if (node.hasOwnProperty(key)) {
        if (!(key in SGFProperties)) continue;
  ...
  for (i = 0; i < gameTree.leaves.length; i++) {
    var subTree = gameTree.leaves[i],
      snapshot;

    snapshot = jrecord.createSnapshot();

    if (!recurseRecord(jrecord, subTree, moveMarks)) return false; // fall through on errors
...
function gameTreeToRecord(gameTree, moveMarks) {
  ...
  if (!recurseRecord(jrecord, gameTree, moveMarks)) return false;
