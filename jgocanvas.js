function JGOCanvas(elem, opt, img) {
    ...
    canvas.onclick = function(ev) {
        var x = Math.floor((ev.pageX-canvas.offsetLeft-marginLeft-padLeft)/opt.grid.x) + opt.view.xOffset,
            ...
            c = new JGOCoordinate(x,y), listeners = self.listeners['click'];
    ...
    for(var i=0; i<opt.view.width; i++) {
        if(opt.coordinates.top)
            this.ctx.fillText(JGO_Letters[i + opt.view.xOffset],
 ...
 * with 'this' referring to JGOCanvas object, with coordinate and event
 * as parameters.
 *
 * @param {String} event The event to listen to, e.g. 'click'.
 * @param {function} callback The callback.
 */
JGOCanvas.prototype.addListener = function(event, callback) {
    this.listeners[event].push(callback);
}

/**
 * A change notifier class that can listen to changes in a JGOBoard and keep
 * multiple JGOCanvas board views up to date.
 *
 * @param {JGOBoard} jboard The board to listen to.
 */
function JGONotifier(jboard) {
    var self = this;
 ...
 *
 * @param {JGOCanvas} jcanvas The canvas to add.
 */
JGONotifier.prototype.addCanvas = function(jcanvas) {
